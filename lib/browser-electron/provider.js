 ...
 * `webContents.debugger` (CDP). The provider itself does not import Electron —
 * it operates through the {@link ElectronBrowserViewHost} seam, which the
 ...
 * @module @deepseek-ai/dsh-browser-electron
 */
...
export class ElectronBrowserProvider {
    ...
    closeTab(session, tabId) {
        ...
        if (index <= s.activeIndex && s.activeIndex > 0 && s.tabs.length > 0)
            s.activeIndex = Math.min(s.activeIndex, s.tabs.length - 1);
        this.showActive(s);
        return Promise.resolve();
    }
