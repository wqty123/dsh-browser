...
export class ElectronBrowserProvider {
    ...
    async click(session, request, signal) {
      ...
      `;
        ...
        const press = (type) => handle.sendCommand('Input.dispatchMouseEvent', { type, x, y, button: 'left', clickCount: 1 });
        const timeoutMs = 30_000;
        try {
    ...
    async runTargetScript(tab, script, timeoutMs, signal, errorCode, errorLabel) {
        const result = await withTimeout(handleSendEvaluate(tab.handle, script), timeoutMs, signal, `${errorLabel} timed out after ${timeoutMs}ms`, () => terminatePage(tab.handle));
        if (!result.ok)
    ...
    async scrape(session, request, signal) {
      ...
      return { ok: true, count: out.length, items: out }
        ...
        const result = await withTimeout(handleSendEvaluate(tab.handle, script), timeoutMs, signal, `browser: scrape timed out after ${timeoutMs}ms`, () => terminatePage(tab.handle));
        if (!result.ok)
    ...
    async handleUserAction(action) {
        ...
        if (s === undefined)
            return; // the session (and its window) is gone
        try {
            switch (action.type) {
                case 'navigate':
    ...
    notifyUserActionError(action, error) {
        const notify = this.host.notifyUserActionError;
