...
export class ElectronBrowserProvider {
    ...
    async click(session, request, signal) {
        ...
        const { handle } = this.activeTab(s);
        signal?.throwIfAborted();
        const press = (type) => handle.sendCommand('Input.dispatchMouseEvent', { type, x: request.x, y: request.y, button: 'left', clickCount: 1 });
        const timeoutMs = 30_000;
        try {
    ...
    async type(session, request, signal) {
        ...
        const timeoutMs = 30_000;
        try {
            await withTimeout(handle.sendCommand('Input.insertText', { text: request.text }), timeoutMs, signal, `browser: type timed out after ${timeoutMs}ms`);
        }
        catch (error) {
