...
function resolveElectronPath() {
    ...
    if (typeof process.versions.electron === 'string') {
        ...
        if (typeof exe === 'string' && exe.length > 0 && existsSync(exe))
            return exe;
    }
    ...
    try {
        ...
        if (typeof entry === 'string' && entry.length > 0) {
            ...
            const binaryReady = existsSync(join(pkgRoot, 'path.txt'))
    ...
    if (hostExe !== undefined)
        return hostExe;
    throw new Error('dsh-builtin-browser: cannot locate the Electron binary. Options: (1) if you are on DSH Desktop, ' +
...
function hostElectronViaProcessTree() {
    ...
    for (let depth = 0; depth < 6 && pid !== undefined && pid > 1; depth++) {
        ...
        if (exe !== undefined && isElectronBinary(exe))
            return exe;
