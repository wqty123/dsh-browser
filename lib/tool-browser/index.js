...
async function ensureSession(browser, key) {
    ...
    if (pending !== undefined)
        return pending;
    const opening = browser.open().then(session => { sessionsByTask.set(key, session); pendingOpens.delete(key); return session; }, error => { pendingOpens.delete(key); throw error; });
